...
class Product(models.Model):
    ...
    def _compute_quantities_dict(self, lot_id, owner_id, package_id, from_date=False, to_date=False):
        ...
        if dates_in_the_past:
            ...
            moves_in_res_past = dict((item['product_id'][0], item['qty_done']) for item in MoveLine._read_group(domain_move_in_done, ['product_id', 'qty_done'], ['product_id'], orderby='id'))
            moves_out_res_past = dict((item['product_id'][0], item['qty_done']) for item in MoveLine._read_group(domain_move_out_done, ['product_id', 'qty_done'], ['product_id'], orderby='id'))
